...
export const getPaymentById = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const row = await queryOne(`SELECT * FROM dbo.payments WHERE id = @id`, { id: data.id });
    ...
    }>(`SELECT family_no, family_name, contact_phone FROM dbo.families WHERE id = @id`, {
      id: row.family_id,
    });
  ...
  .handler(async ({ data, context }) => {
    ...
    if (familyDueId) {
      ...
      const paid = await queryOne<{ approved: number; pending: number }>(
        `SELECT
            ISNULL(SUM(CASE WHEN status = N'approved' THEN amount ELSE 0 END), 0) AS approved,
            ISNULL(SUM(CASE WHEN status = N'pending' THEN amount ELSE 0 END), 0) AS pending
         FROM dbo.payments
    ...
    await execute(
      `INSERT INTO dbo.payments
       ...
       VALUES
         (@familyId, @familyDueId, @createdBy, N'pending', @amount, @mode, @paymentDate, @txnRef, @paidBy, @remarks, @screenshot)`,
      {
...
export const getDashboardSummary = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const payments = await query<{ amount: number; status: string }>(
      `SELECT amount, status FROM dbo.payments
       WHERE (@familyId IS NULL OR family_id = @familyId)`,
      { familyId },
    );
    ...
    let debit = 0;
    if (context.isAdmin) {
      const expenses = await query<{ amount: number }>(`SELECT amount FROM dbo.expenses`);
